...
| 「流量涨了」「这两天为什么涨」「why is traffic up」 | 7 | 先读项目 `.rankup/INDEX.md` 与 `.rankup/decisions.md`，结合线上页面和代码核对已下线功能、重定向及实际能力；再按 GSC 日期/页面/查询与分析来源拆增量，URL 迁移按 [`seo-growth.md`](references/seo-growth.md)「URL 迁移期」判读。未显示查询与未定稿日期单列未知 |
