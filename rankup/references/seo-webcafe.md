...
> **2026-09-11 A/B 复核：`月搜 —` 与「登录态 vs 游客」无关，是取数瞬时失败，`--force` 常能复现出数据。**
> 同日脚本把 kd 之外的工具默认切成经 OpenCLI 驱动登录态浏览器，一度怀疑批量出现大面积
> `—` 是这次改动带偏了 `kd`。实测证伪：`kd` 走的是 `official: true` 分支，一直是 Bearer 令牌
> 直连公开 API（`/kd/api/v1/kd`），**从未接入过浏览器**，`--guest` 只影响配额预检打印，
> 不影响 `kd` 的实际取数路径——同一批词分别跑默认与 `--guest`，六个词结果逐字节一致。
> 用 `kreuzworträtsel`（de/de）实测：第一次裸 GET 返回 `keywordVolume: null`，同一分钟内
> 原样重放两次都稳定返回 `40620`；加 `--force` 也直接拿到 `40620`。说明这是月搜量锚点
> 校验（大概率是 Google Trends 交叉核验，见文件头 2026-09-11 改动记录）偶发没在响应超时
> 内跑完，不是「站点判定这个词真的没量」，也不是本次浏览器路由改动引入的新故障——
> 这条「取数失败当结论」的病在 2026-08-24 的 `bulk rename files` 事故里已经出现过一次。
> 因此本脚本现在在 `summarize()` 里对 `keywordVolume == null && cached === false`
> 的情况多打一句「本次未命中锚点校验，建议 --force 重跑一次再下结论」，不再是裸的 `—`。
> **今天批量出现的 118/131 个 `—`，不能直接当作「无量」处理，逐词 `--force` 复核一遍
> 再定结论**——尤其是要判断做不做站的词。
>
> **2026-09-11 追加：`kd` 单词与 `--batch` 都自动重试一次。** 不用每次都靠人肉重跑——
> 命中 `keywordVolume==null && cached===false` 时，脚本自动带 `force:1` 原样重放一次
> （间隔沿用 `--spacing-ms`，不额外提速，避免把偶发超时误诊成"快点重试就行"），
> stderr 打一行「⏳ <词> → 锚点校验未完成，自动 --force 重试」。**最多重试 1 次**（不循环，
> 不无限重放，多花的是 1 次配额）。两次都拿不到值，才在结果里标
> `anchorPending: true, volume: null, reason: "anchor-pending"`，摘要行也换成
> 「自动 --force 重试后仍未命中锚点校验，reason: anchor-pending」——不再暗示"再试一次就好"，
> 因为已经试过了。`--batch` 走的是同一条主循环，同样生效，不用额外参数。
...
**2026-09-11 更新：默认登录态，游客是显式降级，不是常态。** 上面这条规矩早先只落到了
「查档位」，**没有落到「发请求」**——`kd`/`serp`/`audit`/`money` 等 session 类命令的
实际请求此前一直是 node 侧裸 fetch，从来不会自动走浏览器，只在配额提示里打一段
「你可以手动这样跑」的文字，没人真的手动敲那几行 opencli 命令。三个执行者同一天
批量跑 `kd`，都把这条误导性的「游客 10/日」当成真实上限，刚起步就以为耗尽了——
是 08-22/08-23 那两次事故的重演。现在脚本本身默认经 OpenCLI 驱动本机已登录的
Chrome 执行请求（登录 100/日、VIP 500/日），游客只在显式 `--guest` 或 OpenCLI 不可用
时作为回退，回退会在 stdout 打印醒目警告。下面「httpOnly 会话」一节的手工写法仍然
有效（脚本内部就是这么实现的，调试或写新命令时可以照抄），但日常调用不用再手动跑了。

**同一天证伪：chat 一次多词不会更省配额。** 实测用登录态浏览器往 `/chat/api/chat`
发一条消息问 5 个关键词（nonogram/picross/クロスワード/krzyżówka/kreuzworträtsel）的
搜索量与 KD，`done` 事件报 `toolCalls:9`、`charged:31`——约 6.2 点/词，比直接调用 `kd`
逐词查（1 点/词，且不受浏览器登录状态影响，因为 kd 走的是 Bearer 令牌）贵 6 倍以上。
批量选词**一律用 `kd --batch`**，`chat` 只用于需要模型跨工具综合判断的定性分析，
不用于跑量——想让它一次扫很多词看起来是「省事」，实际是「更贵」。
