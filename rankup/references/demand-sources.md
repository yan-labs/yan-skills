...
[`playbooks/selection.md`](playbooks/selection.md) 闸门 3「付钱雇人 ≠ 会买软件」，本文件不复述。
...
把「被 N 个目录收录」读成流量或销量证据不成立——那要走[第十节](#十候选验证链路)的验证链路。
...
### ②·七·二 哪些词该进同一页：判据是 SERP 首页重不重合

上面那条回答的是「一页能吃多少词」，这条回答「哪些词该被同一页吃掉」。

【经验】判据不是词面像不像，是**把候选词逐个放进搜索引擎，看首页结果重不重合**：

- 搜索意图与首页结果高度相似 → 归一组，选一个主词做页面，其余当相关词在正文里自然覆盖；
- 首页结果**完全不同** → 这是两个主题，分别判断、分别落页，不要因为词面接近就并成一页。

两个方向的错都很常见：把每个候选词都当一个独立页面做，会做出一批互相稀释的近义词页
（段 4「一个关键词对应一个内页」说的是**一个页面只扛一个目标词**，不是「每个候选串都得有一页」）；
反过来，为了把词凑满而把同一个需求写成十篇差不多的文章，是内容农场的形状。
这条判据零配额，和阶段 1 的人眼实勘同一趟做完，不必等取完量再回头分组。
