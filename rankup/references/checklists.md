...
| **Cloudflare Git 集成已连接且首次自动构建成功** | Pages「Git 存储库连接」或 Worker Workers Builds 已连到 `main`；控制台能看到一次成功的自动构建记录；仓库里**没有**给网站部署用的 `.github/workflows/*.yml`（跑测试/lint 的 workflow 不算） | `.rankup/infrastructure.md` | 配置模板见 [`cloudflare-stack.md`](cloudflare-stack.md) §9.1；`grep -rl "wrangler deploy\|pages deploy" .github/workflows/` 应为空 | 一次 |
