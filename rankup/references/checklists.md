...
| 闸门 5 · 哥飞开放 API 数据复核 | `page_coach` / `onpage_audit` 对代表页给出独立结果；与本地事实逐条核实，每条建议有采纳/拒绝记录和理由；原始响应的 `requestId`、`credits.charged`、工具名与市场已保存 | `.rankup/audit.md` + `.rankup/evidence/` | `webcafe-api.mjs tools` / `me` 看价格余额，调用方法见 [`seo-webcafe.md`](seo-webcafe.md)；不使用站内 AI review | 动了页面 |
