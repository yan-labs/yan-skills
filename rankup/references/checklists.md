...
| 闸门 4c · AITDK 全站报告（第三双眼睛） | 可按 sitemap 抽样；报告问题逐条核对当前页面和官方要求，修真实缺陷，误报/不可用项写理由；不以工具满分为通过条件 | `.rankup/evidence/aitdk-full-<date>/`（如运行）+ 判读记录 | `bash <rankup-skill-dir>/scripts/aitdk-opencli.sh <url>`；工具不可用时继续其他可执行验收 | 动了页面 |
