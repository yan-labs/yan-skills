...
| 闸门 5 · 哥飞开放 API 数据复核 | ⏸ | 旧站内 Agent 口径已废弃；按新接口保存原始结果与扣费证据 | 2026-09-25 |
| 闸门 6 · 性能 CWV | ⬜ | — | — |
```
...
| **非英语候选词已过三关** | 每个非英语「做」的词有来源标注且非 `翻译假设` 状态；有目标地区下拉/相关搜索命中，或本地竞品 Title/H1 出现过，两处至少命中一处；locale 精确到语言-地区；预设页面类型与 SERP 实际页面类型一致。英语市场记 N/A | `.rankup/keywords.md` | 三关判法源自【经验】（独立开发者出海经验分享），见 [`playbooks/research.md`](playbooks/research.md#小语种候选词三关与本地竞品取词) | 会过期 |
...
| 需求证据 | 每个进入开发的机会都有两条证据，且**分属不同类别**（用户表达 / 付费行为 / 搜索表达，三类里取两类；同一类里的两个平台只算一条），至少一条是直接信号（GSC 曝光 / Suggest / 持续投放 / 可核验收入）。证据不足的仍标 `RESEARCH`，没有被写成已验证 | `.rankup/decisions.md` | `scripts/demand/` 取数 → [`demand-sources.md`](demand-sources.md) 第十节的候选验证链路；互证口径见 [`demand-discovery.md`](experiences/demand-discovery.md) 三·通则 4 | 一次 |
...
| **D6 · 大数据与路由组件不进入口 bundle** | 构建产物分析（`vite build --report` 或等价）确认入口 chunk 不含题库/条目库这类大数据；路由 loader 数据随 HTML dehydrate，客户端**没有**针对同一份数据的二次 `import()`；每条路由的 `component` 直接在 `createFileRoute(...)({ component })` 里静态引用，**不由工厂函数间接返回**——构建产物分析需确认该组件确实拆进了独立路由 chunk，不在主 chunk 里 | 构建产物分析记录进 `.rankup/audit.md` | 看构建产物体积分布 + 网络面板确认无冗余请求；grep 路由文件确认 `component` 不是由工厂函数返回值赋值 | 一次 |
...
| **图标专项（上线前必过）** | 标准图标集（由同一品牌 logo 源图通过 `make-favicons.mjs` 生成的 `favicon.ico` 内含 16/32/48、`favicon-48.png`、`favicon-96.png`、`favicon-192.png`、`apple-touch-icon.png` 180×180、`icon-512.png`）做齐；首页及各模板的 SSR / 水合后 DOM 所有 icon、shortcut icon、apple-touch-icon 引用，manifest icons 与根 `/favicon.ico` 均完成核对；head 声明 `<link rel="icon" href="/favicon.ico" sizes="48x48">` 与 48/96/192 三个 PNG 图标及 apple-touch-icon、manifest，不引用与 logo 不一致的 SVG；逐个 GET 为 200 且解码为真图（非 HTML），响应类型、声明尺寸与真实尺寸一致，方形、URL 稳定且有 ICO/PNG 搜索回退；每张图经人眼确认属于同一品牌、非脚手架默认图标；16px 可辨认 | 引用清单、响应类型/真实尺寸、实图预览进 `.rankup/audit.md` | [`lifecycle.md`](lifecycle.md) 段 4 · A 节第 4–5a 条；仅有文件或浏览器标签显示正常均不算通过 | 动了图标或 head/manifest |
...
| 闸门 3 · 目标词与内容一致 | 目标搜索意图与页面真实内容一致；目标词自然出现，不设密度通过阈值 | `.rankup/audit.md` | `seo-audit.mjs --sitemap <url> --density-only` 仅辅助发现声明词与实际文本不符；Google 没有最佳关键词密度 | 动了页面 |
| 闸门 4 · GEO / AI Agent 就绪度 | 有逐项基线，`partial`/`failed` 先核对真实环境；`llms.txt` 仅在目标系统确实消费时检查 | `.rankup/agentic/<domain>/<date>.json` + 核实结论进 `audit.md` | `is-agentic.mjs scan <domain> --save` 是第三方诊断，不是 Google 索引或排名证明 | 动了页面 |
| 闸门 4b · GEO 内容形状（AITDK GEO 标签页） | 页面内容对读者有用、来源可核实；表格、FAQ、日期和结构化数据仅在内容与功能适用时提供，AITDK 建议不自动成为 Google 要求 | `.rankup/evidence/aitdk-geo-<date>/`（如运行）+ `.rankup/audit.md` | 检查实际页面和官方搜索指南；预览域的索引限制按设计记录 | 动了页面 |
| 闸门 4c · AITDK 全站报告（第三双眼睛） | 可按 sitemap 抽样；报告问题逐条核对当前页面和官方要求，修真实缺陷，误报/不可用项写理由；不以工具满分为通过条件 | `.rankup/evidence/aitdk-full-<date>/`（如运行）+ 判读记录 | 抽样 URL 用 `bash <rankup-skill-dir>/scripts/aitdk-batch.sh <url...>`（默认 `--window dedicated` 不抢焦点，按真实容量自动降并发，单屏机器上安全串行，见 `seo-box.md`「窗口模式」）；单个 URL 也可以直接 `aitdk-opencli.sh <url>`；工具不可用时继续其他可执行验收 | 动了页面 |
| 闸门 5 · 哥飞开放 API 数据复核 | `page_coach` / `onpage_audit` 对代表页给出独立结果；与本地事实逐条核实，每条建议有采纳/拒绝记录和理由；原始响应的 `requestId`、`credits.charged`、工具名与市场已保存 | `.rankup/audit.md` + `.rankup/evidence/` | 官方 CLI 查看 `tools` / `me` 看价格余额，调用方法见 [`seo-webcafe.md`](seo-webcafe.md)；不使用站内 AI review | 动了页面 |
| 闸门 6 · 性能 / CWV | 抽样首页与主要模板，移动/桌面记录实验室结果；有 CrUX 时记录真实用户 CWV，无数据标未知。优先修用户可感知的瓶颈，逐项判断机会项的实际影响；不以 Lighthouse 满分或单次分数作 SEO 硬闸 | `lhr.json + summary.md`（如运行）与 `.rankup/baseline.md` | `pagespeed.mjs collect <抽样 URL…> --strategy both`；区分实验室数据与现场数据，工具失败不算页面通过 | 动了页面 |
| 封板声明（分数接近满分时） | 剩余建议逐条判「不做」并写理由 | `.rankup/audit.md` | 不封板，团队会持续消耗在零边际收益的项上，而真正的瓶颈动都不动 | 每轮 |
| **改动后验证** | 页面改动重跑受影响闸门；模板、索引配置或全站布局变更扩大到相关页面类型和整站检查，记录覆盖范围与遗漏项 | `.rankup/experiments.md` | 根据 diff 与用户路径选择检查，不用固定全套重跑替代影响分析 | 动了页面 |
| **P0 · 内页有 SEO Brief** | 新增内页在起稿前已有主词/辅助词/搜索意图/页面类型与结构/TDK 草稿，登记进 `.rankup/keywords.md` 该 URL 对应行；非英语页写明是按 Brief 重新生成还是逐句翻译（逐句翻译不通过） | `.rankup/keywords.md` | 见 [`lifecycle.md`](lifecycle.md) 段 4「新增内页 / 新模板的随手清单」P0 | 动了页面 |
...
| **域名黑历史裁决** | 每个候选域名四项都有带日期的证据：官方 `gefei-domain` 的域名历史工具（前世）、Wayback 快照、外链画像（官方 `gefei-domain` 的外链工具或 Ahrefs）、Google `site:` 与品牌名搜索；**成人 / 赌博 / 药 / 被惩罚 / 大量垃圾外链任一命中即否决**，否决的连证据一起记；通过且有前世的标 `has_history: true` | `.rankup/decisions.md` | 做法见 [`lifecycle.md`](lifecycle.md) 段 5 · 5.2。定稿前必查，**不允许「外链多但先用着」** | 会过期 |
...
| **基础安全按用途核验** | HTTPS 收敛、`nosniff`、Referrer-Policy 生效；嵌入、CSP、HSTS 与实际 API 防护按 §8.8 判适用性，缺口已修复或注明阻塞，不用统一模板误伤业务；生产 GET、关键资源/API 类型与相关正常及拒绝路径通过 | `.rankup/audit.md` + `.rankup/infrastructure.md`：变更前后、精确规则/版本、验证与回滚 | [`cloudflare-stack.md`](cloudflare-stack.md) §8.8 | 绑定正式域名后、上线验收前；已上线 review 或改了共享入口/API/安全规则 |
