...
- [〇·六、锚点体量的选择：Semrush 月量的交叉验证是硬规则](#〇六锚点体量的选择用中等量词作锚细分词才能看清形状semrush-月量的交叉验证是硬规则不是可选步骤)（默认锚点、换锚、误差区间、方向性偏差）
- [核心概念](#核心概念)
...
**硬规则**：任何一次「引用 Semrush / Similarweb 月量下结论」的调研，都必须先用本节的锚点法把该月量折算一次、看偏差倍数是否在合理区间，不能只报面板数字。默认锚点用 `gpts`；量级差超过约 10 倍换同量级的第二锚点（详见下方「换锚」）。
...
1. **每一轮调研都要重新拉一次锚点自己最近 12 个月的曲线**，取全年均值做校准基准，**不能沿用上一轮记录的均值**，也不能只截取「最近几周」的读数——上面 2.07 倍的漂移就是这么来的。
2. 拉出来的曲线如果本身是平线（波动幅度小、无明显趋势），直接用；如果像 `gpts` 这样有明显的涨落周期，**仍然可以用，但只用全年均值**，且在报告里注明「锚点自身处于〔高位/低位/回落中〕，折算系数按全年均值口径，非即时口径」。
3. 不要因为锚点不是绝对平线就放弃锚点法——目前没有找到真正稳定不变的中等体量锚点，**「全年均值口径」本身就是应对这个问题的方法，不是退而求其次**。

### 换锚：大词小词不能同框，同量级选第二锚点

**量级差超过约 10 倍时**，Trends 会把小词压成 0–1 的噪声（见〇·五「一个词太冷，整组连坐」的姊妹问题——这里是「大词压死小词」，不是「太冷没有数据」）。`gpts`（US ~5,400）配 1,000–20,000 量级的候选词是安全区间；候选词量级明显更小或更大时换一个同量级的第二锚点，例如实测用过的 `virtual staging`（US 4,400）、`ai ad generator`（US 1,900）。**候选词与两个锚点同框时若某个候选词把锚点的指数压到个位数，说明这一组里有词量级失配，拆组重跑，不要把这一组的折算数字当结论。**

### 折算结果只能当区间读，不能当精确值用

折算月量的误差band 约 **±30–40%**，越小、越冷门的词噪声越大。**12 个月里多数月份指数为 0 的词标「Trends 无法分辨」，不做硬折算**，直接采信 Semrush 原值（长尾修饰词句常见，例如 `best virtual staging for real estate` 这类 "best ... for ..." 形态）。

**过线判据**（未变）：候选词 12 个月均值达到锚点的**一半**就值得看，四分之一到一半算接近但需谨慎。
...
- 处理方式：折算偏差落在 0.5x–2x 算基本吻合；落在这之外、且用不同锚点重复验证方向一致的，按折算区间重新定档，不再采信面板原始数字。
