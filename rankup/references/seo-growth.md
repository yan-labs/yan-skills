...
- **[2026-07-17] meta description 110–160 字符**是 Ahrefs 警告阈值(三把尺里最严的一把,把"太短浪费展示位"也算警告);CJK/阿语按字符数同样适用。本仓两个脚本的区间比这宽且口径不同(`seo-audit.mjs` 50–160 按字符数、`seo-webcafe.mjs string` 70–160 按近似展示宽度),对照表与"说超长时必须点名是哪把尺"的规矩见 [`seo-webcafe.md`](seo-webcafe.md)（数值按各工具自己的口径解释）。
